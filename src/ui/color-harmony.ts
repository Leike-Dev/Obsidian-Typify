...
/** Generate a random hue with balanced saturation/lightness */
function randomBaseHSL(): { h: number; s: number; l: number } {
    return {
        h: Math.random() * 360,
 ...
 */
export function generateAnalogous(count = 5): string[] {
 ...
 */
export function generateComplementary(count = 5): string[] {
    const base = randomBaseHSL();
 ...
 */
export function generateTriadic(count = 5): string[] {
    const base = randomBaseHSL();
 ...
 */
export function generateRandom(count = 5): string[] {
    return Array.from({ length: count }, () => {
        const c = randomBaseHSL();
...
export const HARMONY_GENERATORS: Record<HarmonyType, (count?: number) => string[]> = {
